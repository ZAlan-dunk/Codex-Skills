...
3. PCTR-B: verify SDD confirmation is `confirmed`; read the local SDD Markdown, sidecar, and implementation-plan target path. If a Feishu SDD attachment is registered, also read and validate its attachment identity and containing development-document revision.
