...
请先按项目门禁发送独立命令 `启用OUF`，再使用 brainstorming-unity 为该功能生成一份角色分区 Draft SDD，并使用 writing-unity-plans 生成一份 `Draft / pending-approval` 实施计划，作为联合评审包。
...
- OUF 可以保留自己的 Context Brief / SDD / Plan / Report / Evidence 产物；如当前 PCTR 流程需要，再额外导出一份 PCTR-bound Draft SDD Markdown；计划必须引用该 Draft SDD 的当前身份并保持 pending-approval；不实施代码；
...
- SDD 与计划静态验证通过后，返回 `B-01-runtime-sdd.md`、Context Brief、实施计划路径和上述飞书开发文档的对应功能章节；用户/程序手动上传 SDD 时再登记附件与 OUF 产物路径；不得新建独立飞书 SDD 文档；SDD 与计划确认状态均保持 pending，直到用户联合确认。
