...
| `生成详细 SDD 工件` | After PCTR-B planner confirmation, hand the confirmed `A-02-feature-decomposition.md` to OUF, preserve OUF outputs under `docs/forge-artifacts/`, and register their paths/hashes; produce `B-01-runtime-sdd.md` only when a PCTR-bound SDD snapshot is needed | No |
...
| `开始 <FEATURE-ID> 功能开发` | If planner confirmation is missing/pending, generate/update A-01 and A-02, output the manual upload target for A-01, and remain locked. After planner and SDD/program confirmation pass, generate the detailed route-specific plan | Feishu attachment only while gated; otherwise no |
